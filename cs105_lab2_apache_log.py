# Databricks notebook source exported at Sat, 9 Jul 2016 22:59:52 UTC
# MAGIC %md
...
day_to_host_pair_df = logs_df.select(logs_df.host, dayofmonth("time").alias("day"))
day_group_hosts_df = day_to_host_pair_df.dropDuplicates()
daily_hosts_df = day_group_hosts_df.groupBy(day_to_host_pair_df.day).count().cache()
...
days_with_hosts = daily_hosts_df.select(daily_hosts_df.day).collect()
hosts = daily_hosts_df.select(daily_hosts_df["count"]).collect()
aux_day = []
aux_hosts = []
for idx, day in enumerate(days_with_hosts):
  aux_day.append(days_with_hosts[idx][0])
  aux_hosts.append(hosts[idx][0])
  
days_with_hosts = aux_day
hosts = aux_hosts
...
avg_daily_req_per_host_df = (
  total_req_per_day_df.join(daily_hosts_df, ["day"]).
select(total_req_per_day_df["day"], (total_req_per_day_df["count"]/daily_hosts_df["count"]).alias("avg_reqs_per_host_per_day"))).cache()
...
days_with_avg = (avg_daily_req_per_host_df.select(avg_daily_req_per_host_df.day).collect())
avgs = (avg_daily_req_per_host_df.select(avg_daily_req_per_host_df["avg_reqs_per_host_per_day"]).collect())
aux_day = []
aux_avg = []
for idx, day in enumerate(days_with_avg):
  aux_day.append(days_with_avg[idx][0])
  aux_avg.append(int(avgs[idx][0]))

days_with_avg = aux_day
avgs = aux_avg
print(days_with_avg)
print(avgs)
...
unique_not_found_paths_df = not_found_paths_df.dropDuplicates(["path"])
...
top_20_not_found_df = not_found_paths_df.groupby(not_found_paths_df["path"]).count().sort("count", ascending=False)
...
hosts_404_count_df = not_found_df.groupby(not_found_df["host"]).count().sort("count", ascending=False)
...
errors_by_date_sorted_df = not_found_df.groupby(dayofmonth("time").alias("day")).count().sort("day").cache()
...
days_with_errors_404 = errors_by_date_sorted_df.select(errors_by_date_sorted_df.day).collect()
errors_404_by_day = errors_by_date_sorted_df.select(errors_by_date_sorted_df["count"]).collect()
aux_day = []
aux_error = []
for idx, day in enumerate(days_with_errors_404):
  aux_day.append(days_with_errors_404[idx][0])
  aux_error.append(errors_404_by_day[idx][0])
days_with_errors_404 = aux_day 
errors_404_by_day = aux_error
...
hours_with_not_found = hour_records_sorted_df.select("hour").map(lambda t: t.asDict()["hour"]).collect()
not_found_counts_per_hour = hour_records_sorted_df.select("count").map(lambda t: t.asDict()["count"]).collect()
